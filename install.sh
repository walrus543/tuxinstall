...
####################### TODO ###################################
# si VM alors liste dédiée que paru et prendre chaotis-aur
...
then
    ...
    then
        ...
        then
            ...
            chown -R $USER:users /media/sf_PartageVM/
            check_cmd
        fi
        ...
        then
            ...
            sudo pacman -S --needed --noconfirm virtualbox-guest-utils >> "$log_noroot"
            check_cmd
        fi
        ...
        then
            ...
            sudo pacman-key --recv-key 3056513887B78AEB --keyserver keyserver.ubuntu.com >> "$log_noroot" 2>&1
            sudo pacman-key --lsign-key 3056513887B78AEB >> "$log_noroot" 2>&1
            sudo pacman -U --noconfirm 'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-keyring.pkg.tar.zst' >> "$log_noroot" 2>&1
            sudo pacman -U --noconfirm 'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-mirrorlist.pkg.tar.zst' >> "$log_noroot" 2>&1
            ...
            sudo pacman -Syu --needed --noconfirm >> "$log_noroot"
            check_cmd
        fi
        exit 0;
    else
